...
export const PromoBarChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL}/unit_sold/promo?country=${selectedCountry}&year=${selectedYear}&month=${selectedMonth}`)
  ...
  return (
    <>
      <div className="bg-white p-5 rounded-md shadow-md border border-gray-300 w-full">
        <div className="text-center text-xl mb-10">Promotion Flag on Net Sales - {selectedMonth !== "all" ? selectedMonth : "All Months"}/{selectedYear !== "all" ? selectedYear : "All Years"} - {selectedCountry !== "all" ? selectedCountry : "All Countries"}</div>
